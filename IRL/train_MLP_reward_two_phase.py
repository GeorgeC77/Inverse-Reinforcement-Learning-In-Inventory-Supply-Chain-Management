...
def collect_expert_trajectories(env, expert_policy, n_traj=100, traj_len=200):
    ...
    for _ in range(n_traj):
        ...
        # Two-phase: 随机设置产能 once per episode
        if hasattr(env, 'set_capacity'):
            random_capacity = np.random.uniform(0, 20 * 2)  # 可调范围
            env.set_capacity(random_capacity)
...
for i in range(num_iters):
    obs, _ = venv.reset()
    for e in venv.envs:
        if hasattr(e, "set_capacity"):
            capacity = np.random.uniform(0, 20 * 2)
