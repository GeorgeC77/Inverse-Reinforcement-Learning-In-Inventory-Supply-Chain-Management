...
# 3) 奖励函数
reward_net = ParamInventoryReward(
    observation_space=venv.observation_space,
    action_space=venv.action_space,
    idx_inv_next=3,  # 举例：next_obs 第 lead time 维是 I'
    scale=0.1,
)
...
for i in range(num_iters):
    ...
    print("learned params:", reward_net.get_params())
...
theta = reward_net.get_params()
print(f"h={theta['h']:.4f}, p={theta['p']:.4f}, p/h={theta['p']/theta['h']:.2f}, scale={theta['scale']}")
